...
def build_features(rows: list[dict]) -> tuple[pd.DataFrame, list[str]]:
    ...
    and is LEAK-SAFE: asking_price is never read here, and no feature is derived from
    it. Returns (DataFrame, feature_cols). `district_freq` is a frequency encoding
    computed from the rows themselves (a property-attribute distribution, not the
    target), matching the rental postcode_freq pattern.
    """
    ...
    # District frequency encoding (a property-attribute distribution, NOT the target —
    # mirrors rental postcode_freq; leak-free).
    freq = df["postcode_district"].value_counts(normalize=True)
    df["district_freq"] = df["postcode_district"].map(freq).fillna(0.0)

    # Property-type one-hots + interaction.
