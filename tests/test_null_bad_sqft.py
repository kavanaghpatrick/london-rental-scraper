...
The guarded one-time cleanup NULLs out-of-[150,10000] size_sqft values (the 202 bad
Rightmove OCR rows + the few other-source strays) WITHOUT deleting any row and WITHOUT
touching the recovery columns (size_sqm, floorplan_url) so re-OCR can recover later.
...
_ROWS = [
    ...
    (4, "rightmove", 12415, None, 8, 150000, "http://fp/4.png"),  # bad: >10000
    (5, "rightmove", 10737, None, 7, 28000, None),          # bad: >10000
...
_BAD_PREDICATE = (
    ...
    "AND (size_sqft < 150 OR size_sqft > 10000)"
)
...
def test_baseline_has_six_bad_rows(tmp_path):
    ...
    # ids 1,2,3,4,5 (rightmove) + 7 (savills) = 6 bad across all sources.
...
def test_recovery_columns_preserved(tmp_path):
    ...
    cleanup.run_cleanup(str(db), execute=True)
    conn = sqlite3.connect(db)
    # size_sqm + floorplan_url on the affected ids must be UNTOUCHED so re-OCR can recover.
    row1 = conn.execute("SELECT size_sqm, floorplan_url FROM listings WHERE id=1").fetchone()
    assert row1 == (8, "http://fp/1.png")
    row4 = conn.execute("SELECT size_sqm, floorplan_url FROM listings WHERE id=4").fetchone()
    assert row4 == (None, "http://fp/4.png")
    conn.close()
...
def test_source_filter_only_rightmove(tmp_path):
    ...
    # 5 rightmove bad nulled; the savills bad (id 7) untouched.
    assert res["nulled"] == 5
    conn = sqlite3.connect(db)
