...
- **Coupling:** this is a **clean→retrain** change, not a hot-patch. Sequence: (1) dataeng backfills `postcode_normalized` on a COPY, (2) modeler re-runs `retrain_canonical` → `gen_inference_stats` (regenerates inference freq maps on the cleaned frame — MUST come from the same cleaned data or it mismatches the new pkl) → `gen_feature_parity_golden` (refresh fixture) → re-export to chrome, (3) validate prime-market discrimination improved (expect SW3 freq to drop from 0.88 toward true share), (4) promote (joint). Ping modeler before/after. Until all four happen, leave the column as-is.
- **Bundling (modeler):** my postcode backfill is the natural TRIGGER for ONE clean-data retrain that also bundles two other deferred model items — `is_social_housing` (estate-regex-only / leak-free) and `is_premium_agent` (case-fix). Lead already agreed to defer + bundle all three. So when scheduled, step 1 kicks off a single retrain covering all three, not a postcode-only pass.
