/**
 * XGBoost Predictor — shared by the Chrome extension (browser) AND serving's
 * Next.js /api/predict route (Node). Pure / Node-safe: no window/document/location.
 * Loads the exported XGBoost JSON model and runs predictions identically in both
 * environments, so client and server never drift.
 */

// Module-scope cache of parsed {model, features} keyed by URL pair. On a Vercel
// serverless function this makes WARM invocations reuse the already-parsed 11MB
// model instead of re-fetching/re-parsing it. Single model source: the same
// GitHub-raw URL the extension uses (post-push, main = canonical v20).
const _XGB_MODEL_CACHE = (typeof globalThis !== 'undefined'
...
class XGBoostPredictor {
  ...
  async load(modelUrl, featuresUrl) {
    ...
    // (warm serverless invocation, or a second predictor in the same page).
    const cacheKey = `${modelUrl}|${featuresUrl}`;
    const cached = _XGB_MODEL_CACHE.get(cacheKey);
    if (cached) {
    ...
    this.loaded = true;
    _XGB_MODEL_CACHE.set(cacheKey, { model: this.model, features: this.features });
  ...
  predictTree(tree, features) {
    ...
    while (true) {
      ...
      // XGBoost stores split_conditions as float32 and evaluates the split in
      // float32. Reading them as JS float64 makes boundary cases (value ≈ threshold)
      // branch the wrong way, accumulating ~0.7% error over 1500 trees. Round BOTH
      // sides to float32 (Math.fround) so the comparison matches native XGBoost.
...
const XGBFeatures = {
  ...
  // Ultra-prime addresses - highest tier (drives is_ultra_prime_address)
  ...
  // V16 ULTRA-PRESTIGE STREETS — drives is_ultra_luxury_address (a DISTINCT feature
  // from is_ultra_prime_address). Must match Python ULTRA_PRESTIGE_STREETS verbatim.
  ...
  // Postcode frequency = v20 TRAINING frequencies (count/N over the 5,147-row train
  // set), baked from output/rental_model_canonical_inference.json (gen_inference_stats.py).
  ...
  POSTCODE_FREQ: { 'UNKNOWN': 0.1446500744522442, 'SW7': 0.08402467560093597, 'SW3': 0.08285471176345459, 'NW8': 0.07083599234205488, 'W8': 0.05435013826845352, 'NW3': 0.0538183365241438, 'SW5': 0.04690491384811742, 'W2': 0.04296958094022548, 'NW1': 0.04233141884705382, 'SW1X': 0.0408423739629866, 'SW6': 0.03850244628802382, 'SW10': 0.03743884279940438, 'SW1W': 0.027547330355243566, 'W11': 0.02637736651776218, 'SW11': 0.02318655605190385, 'W14': 0.021591150818974687, 'W1H': 0.016485854073601362, 'W1U': 0.016166773027015528, 'W1J': 0.015847691980429696, 'W1K': 0.01372048500319081, 'W1W': 0.008508827908955541, 'W10': 0.008508827908955541, 'SW1P': 0.007870665815783875, 'W6': 0.006594341629440545, 'W1G': 0.006487981280578601, 'SW18': 0.0048925760476494365, 'SW1V': 0.004679855349925548, 'W12': 0.0045734950010636035, 'W1': 0.0042544139544777706, 'SW19': 0.004041693256753882, 'SW15': 0.0039353329078919376, 'W1T': 0.003190810465858328, 'SW13': 0.002765369070410551, 'NW11': 0.002765369070410551, 'SW12': 0.0026590087215486065, 'W1S': 0.0025526483726866626, 'W1B': 0.0025526483726866626, 'SW1E': 0.0021272069772388853, 'SW14': 0.0014890448840672197, 'NW6': 0.0014890448840672197, 'SW1': 0.0014890448840672197, 'E14': 0.0014890448840672197, 'W9': 0.001169963837481387, 'SW1H': 0.001169963837481387, 'SW17': 0.001169963837481387, 'E1W': 0.0010636034886194426, 'NW2': 0.0009572431397574984, 'SW1A': 0.0007445224420336099, 'W1F': 0.0007445224420336099, 'W1D': 0.0006381620931716656, 'SW1Y': 0.0006381620931716656, 'W13': 0.0003190810465858328, 'NW5': 0.00021272069772388852, 'SE1': 0.00021272069772388852, 'N6': 0.00021272069772388852, 'E16': 0.00021272069772388852, 'SW16': 0.00010636034886194426, 'B8': 0.00010636034886194426, 'W1X': 0.00010636034886194426, 'NW10': 0.00010636034886194426, 'GU25': 0.00010636034886194426, 'SW8': 0.00010636034886194426, 'E1': 0.00010636034886194426, 'TW8': 0.00010636034886194426, 'WC2N': 0.00010636034886194426, 'EC3R': 0.00010636034886194426, 'TW9': 0.00010636034886194426, 'default': 0.00010636034886194426 },

  // Postcode area frequency = v20 TRAINING frequencies (same source as POSTCODE_FREQ).
  POSTCODE_AREA_FREQ: { 'SW': 0.4211869814932993, 'W': 0.25771112529249096, 'NW': 0.17251648585407361, 'UNKNOWN': 0.1446500744522442, 'E': 0.002871729419272495, 'TW': 0.00021272069772388852, 'SE': 0.00021272069772388852, 'N': 0.00021272069772388852, 'EC': 0.00010636034886194426, 'WC': 0.00010636034886194426, 'GU': 0.00010636034886194426, 'B': 0.00010636034886194426, 'default': 0.00010636034886194426 },
  ...
  // Get source quality. Python maps the `source` field exactly
  // ({savills:4, knightfrank:4, chestertons:3, foxtons:2, rightmove:1}).fillna(2).
  // PRIMARY: explicit `source` field (the contract). FALLBACK: detect from pageUrl
  // for the browser path where only the URL is known. Default = 2 (Python fillna).
  getSourceQuality(pageUrl, source) {
    if (source) {
      const key = String(source).toLowerCase().replace(/\s+/g, '');
      for (const [s, quality] of Object.entries(this.SOURCE_QUALITY)) {
        if (s.replace(/\s+/g, '') === key) return quality;
      }
      return 2; // Python .fillna(2) for an unknown but present source
    }
    if (!pageUrl) return 2;
    const urlLower = pageUrl.toLowerCase();
    for (const [s, quality] of Object.entries(this.SOURCE_QUALITY)) {
      if (urlLower.includes(s.replace(/\s+/g, ''))) return quality;
    }
    return 2; // default for unknown sources (match Python fillna)
  },
  ...
  // Get postcode frequency from the baked training map (hasOwnProperty so a real
  // 0-frequency wouldn't fall through to default).
  getPostcodeFreq(postcodeDistrict) {
    return Object.prototype.hasOwnProperty.call(this.POSTCODE_FREQ, postcodeDistrict)
      ? this.POSTCODE_FREQ[postcodeDistrict]
      : this.POSTCODE_FREQ['default'];
  },

  // Get postcode area frequency from the baked training map.
  getPostcodeAreaFreq(postcodeDistrict) {
    ...
    return Object.prototype.hasOwnProperty.call(this.POSTCODE_AREA_FREQ, area)
      ...
      : this.POSTCODE_AREA_FREQ['default'];
  },
  ...
  buildFeatures(data) {
    ...
    // Floor information. PRIMARY: explicit floor flags passed in the request
    // (the /api/predict contract + page-extracted floors). FALLBACK: derive from
    // OCR floorplan text. Python's FE consumes these flags directly, so when the
    // caller supplies them we must use them verbatim (don't override with OCR).
    const hasExplicitFloors = (
      data.floor_count != null || data.has_basement != null || data.has_ground != null ||
      data.has_first_floor != null || data.has_second_floor != null ||
      data.has_third_floor != null || data.has_fourth_plus != null ||
      data.has_roof_terrace != null
    );
    let floors;
    if (hasExplicitFloors) {
      const fc = Number(data.floor_count) || 0;
      floors = {
        has_basement: Number(data.has_basement) || 0,
        has_ground: Number(data.has_ground) || 0,
        has_first_floor: Number(data.has_first_floor) || 0,
        has_second_floor: Number(data.has_second_floor) || 0,
        has_third_floor: Number(data.has_third_floor) || 0,
        has_fourth_plus: Number(data.has_fourth_plus) || 0,
        has_roof_terrace: Number(data.has_roof_terrace) || 0,
        floor_count: fc,
        is_multi_floor: fc >= 2 ? 1 : 0,
      };
    } else {
      // Extract floor information from OCR text (matches Python FloorplanExtractor)
      floors = this.extractFloors(ocrText);
    }
    ...
    const sourceQuality = this.getSourceQuality(pageUrl, data.source);
    ...
    // is_ultra_luxury_address is its OWN feature (Python ULTRA_PRESTIGE_STREETS),
    // NOT the same as is_ultra_prime_address (ULTRA_PRIME_ADDRESSES).
    const addrLowerForLuxury = (address || '').toLowerCase();
    const isUltraLuxuryAddress = this.ULTRA_PRESTIGE_STREETS.some(st => addrLowerForLuxury.includes(st)) ? 1 : 0;
    ...
    // Calculate floor-related features. Python's training default for floor_count is
    // 0 (training median) — NOT 1. floor_size_interaction follows from it.
    const floorCount = floors.floor_count || 0;
...
// Universal export: works as a browser content-script global (extension) AND as a
// Node/CommonJS module (serving's Next.js /api/predict route imports the SAME source,
// so the client and server feature builder can never drift — JS SSOT mirroring the
// Python canonical_predict discipline).
...
if (typeof module !== 'undefined' && module.exports) {
  // Node / Next.js server route
  module.exports = { XGBoostPredictor, XGBFeatures };
}
